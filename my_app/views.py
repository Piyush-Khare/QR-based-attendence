...
def takeAttendence(request):
  if request.method == 'POST':
    ...
    if Attendence.objects.filter(date=str(date.today()),branch=details['branch'],semester=details['semester'],section=details['section'],period=details['period']).count()!=0:
      messages.error(request, "Attendence already recorded.")
      return redirect('home')
    else:
      students = Student.objects.filter(branch = details['branch'], semester = details['semester'], section = details['section'])
      ...
      for student in students:
        if str(student.registration_id) in names:
         #attendence = Attendence(Faculty_Name = request.user.faculty, 
          ...
          section = details['section']
          attendence.save()
      attendences = Attendence.objects.filter(date = str(date.today()),branch = details['branch'], semester = details['semester'], section = details['section'],period = details['period'])
